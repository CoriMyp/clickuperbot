...
def convert_usernames_to_members(usernames: List[str]):
    ...
    for username in usernames:
        member_name = execute("SELECT name FROM members WHERE username = ?", (username,)).fetchone()
        if member_name:
...
def folder() -> Optional[dict]:
    folder_name = execute("SELECT value FROM data WHERE key=?", ("folder",)).fetchone()[0]
    response = requests.get(
...
def new_task(name: str, description: str, users: List[str], deadline: str):
    ...
    if deadline != "":
        try:
            due_day = dt.strptime(deadline, "%d.%m").replace(year=dt.now().year).timestamp()
        except ValueError:
